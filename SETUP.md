...
| `X_API_KEY` / `X_API_SECRET` | OAuth 1.0a Consumer Key/Secret (used by Connect via OAuth 1.0a fallback) |
...
2. **OAuth 2.0 Client ID + Client Secret are valid** for the current app type (Web App / confidential).  
   GrowLab probes the token endpoint with the stored secrets; `invalid_client` means the Client ID/Secret pair is rejected by X (see expanded probe).
...
8. Retry **Connect X** (or `GET /oauth/x/probe` while logged in — expect `credentials_ok: true`).
...
If `/oauth/x/probe` stays `credentials_ok: false` with `invalid_client` after a confirmed Web App Client ID (`:ci`) + Client Secret paste, use:

```
https://growlab.darylbleach.workers.dev/oauth/x/start-oauth1
```

That uses Worker secrets `X_API_KEY` / `X_API_SECRET` (OAuth 1.0a Consumer Key/Secret). Callback is the same `/oauth/x/callback`.
