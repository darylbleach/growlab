  ...
  if (apiKey && apiSecret) {
    try {
      ...
      oauth1Probe = {
        ...
        error: redactOauthMaterial(msg).slice(0, 240),
      };
    }
  }
...
async function handleOauth1Callback(
  ...
  try {
    const clientIp = c.req.header("cf-pseudo-ipv4") || c.req.header("cf-connecting-ip");
    const access = await oauth1AccessToken(c.env, oauthToken, tokenSecret, oauthVerifier, clientIp);
...
function redactOauth(message: string): string {
  return redactOauthMaterial(message);
}
