...
import { buildAccessTokenExchange, encodeSignaturePlus, isIpv4, oauth1ClientIpHeaders, oauth1RealIpHeader, oauth1Sign, oauth1SigningKey } from "./oauth1.ts";
...
test("encodeSignaturePlus percent-encodes plus signs inside oauth_signature only", () => {
  const header = 'OAuth oauth_consumer_key="ck", oauth_signature="abc+def/g==", oauth_token="t+ok"';
  assert.equal(
    encodeSignaturePlus(header),
    'OAuth oauth_consumer_key="ck", oauth_signature="abc%2Bdef/g==", oauth_token="t+ok"',
  );
});

test("oauth1ClientIpHeaders supports x-forwarded-for and both", () => {
  assert.equal(isIpv4("8.8.8.8"), true);
  assert.equal(isIpv4("2001:db8::1"), false);
  assert.deepEqual(oauth1ClientIpHeaders("8.8.8.8", "both"), {
    "x-real-ip": "8.8.8.8",
    "x-forwarded-for": "8.8.8.8",
  });
  assert.deepEqual(oauth1ClientIpHeaders("8.8.8.8", "none"), {});
});
